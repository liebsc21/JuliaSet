...
Init<T>::Init(int argc, char* argv[]){
  ...
  /* translate all positional program options
     to "card" program options, i.e. "RunCards/Run_2.ini" is parsed as
     "--card RunCards/Run_2.ini" */
  ...
  /* vm is similar to std::map.
     It stores all values of options, which have been specified */
  ...
  /* retrieve values of options from vm */
  m_debug = vm["debug"].as<const int>();
  string card = vm["card"].as<string>();
  /* access options, which were given in run card */
